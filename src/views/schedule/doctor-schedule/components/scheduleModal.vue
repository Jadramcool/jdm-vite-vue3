...
const doctorRef = ref<any>(null);

const { editFormSchemas } = useDoctorScheduleSchema();

const [registerForm, { setFieldsValue, resetFields, submit, updateSchema, getComponentInstance }] =
  useForm({
    labelWidth: 120,
    gridProps: { cols: 1 },
...
const [register, { closeModal, setModalProps }] = useModalInner(async (data) => {
  ...
  doctorRef.value = await getComponentInstance('dateType');
});
