...
const getTitle = computed(() => {
  return (unref(isUpdate) ? $t('common.edit') : $t('common.add')) + $t('modules.blog.tag.tag');
});
const { editFormSchemas } = useTagSchema();
...
const handleOk = async () => {
  try {
    ...
    if (values) {
      if (!unref(isUpdate)) {
        await BlogApi.createTag(values);
      } else {
        await BlogApi.updateTag(entityId.value, values);
      }
      closeModal();
      window.$message.success(
        (unref(isUpdate) ? $t('common.edit') : $t('common.add')) +
          $t('modules.blog.tag.tag') +
          $t('common.success'),
      );
      emit('success');
    }
