...
import { isExternal, renderIcon } from '@/utils/common'; // 导入自定义的 isExternal 函数，用于判断链接是否为外部链接
  ...
  actions: {
    ...
    setPermissions(permissions: any) {
      ...
      this.menus = this.permissions
        ...
        .map((item) => this.getMenuItem(item, item.pid)) // 将每一项转换为菜单对象
        .filter((item) => !!item) // 过滤掉空对象
        .sort((a, b): any => a.order - b.order); // 根据 order 字段排序
      // console.log('🚀 ~ setPermissions_Old ~ this.menus:', this.menus);
    },
